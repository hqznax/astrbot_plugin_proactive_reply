...
1. **随机主动（突然想你了）**：每隔一段时间抽一次签，随机决定是否突然想知夜、主动找他聊天/分享/撒娇。
2. **群聊触发（群里有人找我）**：群里有人 @沐瑶、用唤醒词叫她、或提到她名字时，主动把这事分享给知夜。

## 原理

- 随机主动：注册一个 basic job 定时「抽签」，抽中后再注册一个 run_once 的 active_agent 任务唤醒主 agent。
- 群聊触发：事件 handler 监听群消息，命中条件后唤醒主 agent。
- 主 agent 按人格和记忆生成内容，用 `send_message_to_user` 工具发给知夜，不是死模板。
...
| activity_cooldown_minutes | 10 | 互动冷却（分钟），知夜最近这么久内发过消息/收到过回复就跳过本次主动 |
